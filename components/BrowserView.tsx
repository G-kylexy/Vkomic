...
const BrowserView: React.FC<BrowserViewProps> = ({
  ...
  // New optimization: Map<ID, VkNode> for O(1) lookups
  ...
  const activeDownloadsInView = React.useMemo(() => {
    ...
    // Optimization: iterate downloads (O(M)) instead of fileNodes (O(N))
    // This is much faster when N (files in view) >> M (active downloads)
    downloads.forEach((d) => {
    ...
    return active;
  }, [fileNodes.length, fileNodesMap, downloads]);
