...
const DownloadsView: React.FC<DownloadsViewProps> = ({ downloads, pauseDownload, resumeDownload, cancelDownload, retryDownload, syncedData }) => {
  ...
  const stats = useMemo(() => {
    ...
    // Calcul approximatif des séries indexées (nombre total de noeuds parcourus)
    let seriesCount = 0;
  ...
  const getVolumeLabel = (title: string) => {
    const match = title.match(/T(\d+)/i) || title.match(/#(\d+)/) || title.match(/Vol\.?(\d+)/i);
  ...
  return (
    <div className="flex-1 p-8 flex flex-col pt-6 animate-fade-in overflow-y-auto custom-scrollbar">
      ...
      {/* Section 1: Vue d'ensemble (Stats) */}
      <div className="mb-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-[#131926] border border-[#1e293b] rounded-xl p-6 flex flex-col justify-center shadow-lg">
            <span className="text-4xl font-bold text-white mb-2">{stats.indexed}</span>
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">{t.downloads.statsIndexed}</span>
          </div>

          {/* Card 2 */}
          <div className="bg-[#131926] border border-[#1e293b] rounded-xl p-6 flex flex-col justify-center shadow-lg">
            <span className="text-4xl font-bold text-white mb-2">{stats.downloaded}</span>
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">{t.downloads.statsDownloaded}</span>
          </div>

          {/* Card 3 */}
          <div className="bg-[#131926] border border-[#1e293b] rounded-xl p-6 flex flex-col justify-center shadow-lg">
            <span className="text-4xl font-bold text-white mb-2">{stats.inProgress}</span>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr>
                  ...
                  <th className="py-4 px-6 bg-[#0f1523] text-xs font-bold text-slate-500 uppercase border-b border-slate-800 tracking-wider w-24">
                    {t.downloads.tableVolume}
                  </th>
                  <th className="py-4 px-6 bg-[#0f1523] text-xs font-bold text-slate-500 uppercase border-b border-slate-800 tracking-wider w-64">
                    {t.downloads.tableStatus}
                  </th>
                  <th className="py-4 px-6 bg-[#0f1523] text-xs font-bold text-slate-500 uppercase border-b border-slate-800 tracking-wider w-32">
                    {t.downloads.tableSize}
                  </th>
                  <th className="py-4 px-6 bg-[#0f1523] text-xs font-bold text-slate-500 uppercase border-b border-slate-800 tracking-wider w-24 text-right">
