...
const DesktopDownloadRow = React.memo(({ d, t, pauseDownload, resumeDownload, cancelDownload, retryDownload, openFolder }: RowProps) => {
...
const MobileDownloadRow = React.memo(({ d, t, pauseDownload, resumeDownload, cancelDownload, retryDownload, openFolder }: RowProps) => {
  const isCompleted = d.status === "completed";
  const isDownloading = d.status === "downloading";
  const isPaused = d.status === "paused";
  const isCanceled = d.status === "canceled";
...
const DownloadsView: React.FC<DownloadsViewProps> = ({
  ...
  // Pré-calcul des données triées
  // OPTIMIZATION: Return DownloadItem[] directly to preserve object references for unchanged items.
  // This allows React.memo in child components to work correctly.
